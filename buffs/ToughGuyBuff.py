...
class ToughGuyBuff(Buff):
    ...
    async def buff_action(self):
        ...
        if not expired:
            ...
            initial_time = int(user['current__time'] / (1 + self.increase_coefficient) ^ self.duration)
            ...
            await db.update_parameter(parameter='current__time',
                                      new_value=initial_time,
                                      user_id=user['user_id'],
                                      chat_id=user['chat_id'])
            await bot.send_message(chat_id=user['chat_id'],
                                   text='Поздравляю, ' + user['name'] + '! Ты уверенно справился с испытанием '
                                        + self.name + ' и заслужил право на 1 день отдыха! Вернул твое время на'
                                                      ' изначальное значение в ' + str(initial_time) + ' секунд.')
